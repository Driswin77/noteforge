...
How it works
Upload a PDF or PPTX slide deck
Extraction — Python (pdfplumber / python-pptx) pulls raw text per slide
AI restructuring — Google Gemini groups slides into logical topic sections, rewrites the content into clear notes (not copy-pasted bullets), and generates a glossary + 5-question quiz per section
View & study — browse notes by section, test yourself with the interactive quiz, and export to PDF
...
│       ├── models/
│       └── routes/
└── extractor/        Python extraction + LLM scripts
    ├── extract.py           # PDF/PPTX → raw slide JSON
    ├── generate_notes.py    # raw slides → structured notes via Gemini
